...
export async function requestAssetAnalysisConfig(context: { requirement: string; assetName: string; fields: Array<{ name: string; type: string }>; rowCount: number }, bindings: LlmBindings, fetcher: typeof fetch = fetch, diagnostic?: SensitiveDebugLogger, timeoutMs = ANALYSIS_LLM_TIMEOUT_MS): Promise<ReportConfig> {
  ...
  try { response = await fetcher(`${bindings.LLM_BASE_URL.replace(/\/$/, '')}/chat/completions`, { method: 'POST', headers: { authorization: `Bearer ${bindings.LLM_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify({ model: bindings.LLM_MODEL, messages: [{ role: 'system', content: rules }, { role: 'user', content: JSON.stringify(context) }], response_format: { type: 'json_object' }, }), signal: AbortSignal.timeout(timeoutMs) }) } catch (error) { if (isTimeoutError(error)) throw new LlmClientError('LLM_REQUEST_TIMEOUT', `分析规则生成超过 ${timeoutMs / 1000} 秒`); throw new LlmClientError('LLM_REQUEST_FAILED', '分析规则生成请求失败') }
  ...
  try {
    ...
    if (isTimeoutError(error)) throw new LlmClientError('LLM_REQUEST_TIMEOUT', `分析规则响应读取超过 ${timeoutMs / 1000} 秒`)
...
export async function requestAssetMetadataSuggestion(
  ...
  diagnostic?: SensitiveDebugLogger,
) {
  if (bindings.ENVIRONMENT === 'test' || bindings.ENVIRONMENT === 'unit-test') {
  ...
  try {
    ...
    diagnostic?.info('元数据建议模型完整响应', payload)
