...
describe('数据资产分析 API', () => {
  ...
  it('创建经校验的分析并在资产历史中返回它', async () => {
    const created = await authenticatedRequest(`/api/assets/${assetId}/analyses`, {
      method: 'POST',
    ...
    const listed = await authenticatedRequest(`/api/assets/${assetId}/analyses`, {}, env)
  ...
  it('返回冻结的规则和用于渲染的数据行，并禁止跨资产访问', async () => {
    ...
    const detail = await authenticatedRequest(`/api/assets/${assetId}/analyses/${id}`, {}, env)
    ...
    expect((await authenticatedRequest(`/api/assets/${otherAssetId}/analyses/${id}`, {}, env)).status).toBe(404)
  })

  it('以明确原因记录模型生成失败的分析', async () => {
    const response = await authenticatedRequest(`/api/assets/${assetId}/analyses`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ requirement: '不支持的分析需求' }),
    }, env)
